...
public class Main {
	...
	private static ReaderFactory[] readers;
	public static Reader[] instances;
	...
	public static void main(String[] args) {
		...
		fileSaveLogItem.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				try {
					...
					System.out.println(new Date().toLocaleString() + "> Log saved");
					fileWriter.write(console.getText());
					fileWriter.close();
					
				} catch (IOException e) {
					System.out.println(new Date().toLocaleString() + "> " + e);
				}
			}
			@Override
		...
		class Open implements SelectionListener {
			public void widgetSelected(SelectionEvent event) {
				...
				System.out.println(new Date().toLocaleString() + "> Configuation file loaded: " + selected);
			}
	...
	public static void startReading(){
		...
		executor = Executors.newFixedThreadPool(numberOfReaders);

		readers = new ReaderFactory[numberOfReaders];
		instances =  new Reader[numberOfReaders];
		int argsCount = 0;

		for(int i = 0 ; i < readers.length ; i++){
			executor.execute(new ReaderFactory(config[argsCount], config[argsCount +1], config[argsCount +2], i, serverIP.getText(), timeDiff));
			argsCount += 3;
		}
	}
